/**
 * Trip Picking List — single-page summary table, one row per order
 *
 * Columns: Sale No. | Customer | Team | Amount | Total VAT
 * Bottom: Total row + customer count
 *
 * Same layout as trip-summary-template.ts
 */
...
function fmtDateTimeUK(v?: string | null): string {
  if (!v) return ''
  const d = new Date(v)
  if (Number.isNaN(d.getTime())) return ''
  const dd = String(d.getDate()).padStart(2, '0')
...
export function generateTripPickingHtml(data: TripPrintData): string {
  ...
  const now = fmtDateTimeUK(new Date().toISOString())
