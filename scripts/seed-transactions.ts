...
async function main() {
  ...
  for (let i = 0; i < assigns.length; i++) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        items: lines.map(l => ({ productId: l.productId, productName: l.productName, spec: '', price: l.unitPrice, quantity: l.orderedQty, subtotal: l.subtotal })),
        totalAmount,
  ...
  function buildRestaurant(o: BuiltOrder, delivered: boolean) {
    return {
      ...
      orderIds: [o.id],
      items: o.lines.map(l => ({ productId: l.productId, productName: l.productName, spec: '', price: l.unitPrice, quantity: l.orderedQty, subtotal: l.subtotal })),
      delivered,
      returns: [],
      pods: [],
      cargoVerified: true,
