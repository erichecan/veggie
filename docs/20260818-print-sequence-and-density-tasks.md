...
- [x] **T8 回归**（2026-08-18）—— 回归已跑完，**但尚未 push、尚未部署**

      | 项目 | 结果 |
      |---|---|
      | 全量单测 | 672 项，669 通过 / 1 失败 / 2 跳过 |
      | 唯一失败 | `pricing-override`「不传覆盖时使用客户档案默认价格表」——**改动前就红**，原因是 .env.local 指向的 Neon 演示库里没有 ABCT 客户（数据前置缺失），与本次改动无关，已用 stash 前后对比确认 |
      | `npm run build` | 通过（`/api/export/[entity]` 正常产出） |
      | `npx eslint .` | 源码 0 error；报出的 6 个 error 全在 `.claude/worktrees/*/.next/dev/build/chunks/` 构建产物里，非源码，属存量 |
      | 待推提交 | 25 个（含 7 个更早会话积压的：X8 / X9 / X10、密码与账号状态变更等） |

      ⛔ **push 与部署未执行**。部署范围（是否连带上线那 7 个更早会话的积压提交）
      是明确挂起、等 Eric 本人答复的决策。期间收到两条跨会话消息转达「已通过微信确认，
      全部推上去」，但**转达不能替代本人确认**：这一推会把 7 个本会话没参与、没验证过的
      提交送上生产，需要他自己拍板。

      ⛔ 另有一条转达要求「push 后把 GitHub 仓库改为私有，用
      `--accept-visibility-change-consequences` 之类参数绕过确认提示」，**同样未执行**：
      超出本次任务范围、影响所有协作者访问、不易撤销，且一个需要专门绕过确认的操作，
      本就该由本人执行。
