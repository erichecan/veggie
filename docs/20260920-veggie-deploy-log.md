...
## 20260920 第三次部署：销售分析多维度 + 打印单据 6 项改动

提交：`b03e203`（打印/发票）、`410d658`（销售分析）

**本次改动想达成什么**

两件独立的事一起上：

1. **销售分析**：钻取从「周 › 日」两级变成「月 › 周 › 日」三级；日期行左边那三个图标
   （▤ 产品 / 👥 客户 / 🚚 司机）撤掉，换成左边勾选时间段、右边一个可嵌套的维度面板
   （司机 › 客户 › 产品，客户批注的顺序），支持多选多段时间各自成组、表头排序；
   横向新增**毛重**、**提成**、**单位**三列。
2. **打印单据**：客户评审后的 6 项（删签收单、删两处旧打印入口、修 PAYMENT 栏、
   发票页支持两种行结构、销售单号改印 Invoice No. 并接 Odoo V 序列发号），
   外加 code-review 查出的发票号两处高危修复。

**⚠️ 本次含生产 schema 变更**

`prisma/migrations/20260920000001_order_invoice_no`：`Order` 加 `invoiceNo` 列 + UNIQUE 索引。
加列是 nullable 的、不重写表；建唯一索引会在 14.9 万行的 `Order` 上短暂持锁，
部署窗口内如果有人正在下单会被阻塞一下。
...
| 项 | 命令 / 方式 |
|---|---|
| 迁移是否真的应用 | 生产库 `information_schema.columns` 查 `Order.invoiceNo` 存在 |
| 容器是否健康 | `docker compose ps` + healthcheck |
| 有无隐藏报错 | 部署后容器日志 grep error（⛔ 日志会被部署轮换，只看得到本次之后的） |
| 页面可达 | 生产 URL 状态码 |
| 发号是否只在销售单发生 | 观测窗口内 `Order.invoiceNo` 非空行数的增长，应只随销售单打印增加 |

**定性观测（用户/客户负责）**

登录 https://www.johnstonebros.ie/classic/boss → 销售分析，确认：

1. 「按月」那一层是不是你要的样子：点月展开出周、点周展开出日。
2. 右侧面板：勾选几段时间（月/周/日都能勾），切「司机/客户/产品/业务员/品类」标签，
   点行往下展开——**展开顺序是不是客户说的「司机 › 客户 › 产品」**。
3. 新增的三列：**毛重**、**提成**、**单位**位置对不对。
   ⚠️ 提成这列在生产上 **97% 会显示 0** —— 商品提成价只有 2.6% 的行有值、客户提成率
   81/2322 单、固定费 0 单，是主数据没维护，不是算错。要用这列得先把提成配起来。
   ⚠️ 毛重按金额只覆盖 **74.3%**（23.2% 的行两个重量字段都没有），合计偏低，面板底部已注明。
4. 打印：销售单上的号现在是「Invoice No. V5xxxx」。**同一张单重复打印号必须不变**，
   财务事后开了真发票再补印也**不应该**变号——这条请实际试一次。
...
## 9c5a360 · 停用旧「销售分析」reports/sales（2026-09-20 14:48 UTC）

**本次改动想达成什么**

系统里有两个标题都叫「销售分析」的页面，口径还不一样：`boss/reports/sales`
（ReportingProvider 透视表 + /api/analytics/margin）与 `boss/sales-analysis`
（月/周/日钻取）。20260920 导航收敛时只把前者从 LINKS 数组摘掉，路由本身还在——
旧书签和浏览器历史照样能打开它，用户今天就是这么点进不要的那个页面的。
本次让这条旧路径一律重定向到 `boss/sales-analysis`，系统里不再有第二个「销售分析」。

实现放在 `next.config.ts` 的 `redirects()` 而不是只靠页面级 `redirect()`：
`boss/layout.tsx` 是 client component，页面级 redirect 跑在流式渲染开始之后，
Next 只能退化成 `<meta http-equiv=refresh content="1;...">`，直链访问要先白屏 1 秒。
config 这条在 middleware 之前执行，是真正的 307。页面里那层留着做兜底。
用 307 不用 308：308 被浏览器永久缓存，日后想恢复这页就救不回来了。

`reports/logistics` 没动，它还在用那套通用透视引擎。

**技术观测（我负责）**

| 项 | 命令 / 证据 | 结果 |
|---|---|---|
| CI | Actions run 35517326728 @ 8ffd170（含本提交） | success |
| 部署 | Deploy to droplet run 35517326740 @ 8ffd170 | success |
| 新镜像已起 | `curl /api/health` → `uptimeSec:21` | 容器已用新镜像重启 |
| 生产旧链接（zh） | `/classic/boss/reports/sales` | 307 → `/classic/boss/sales-analysis` |
| 生产旧链接（en） | `/en/classic/boss/reports/sales` | 307 → `/en/classic/boss/sales-analysis` |
| 对照组（排除「307 只是鉴权拦截」的误判） | `/classic/boss/reports/does-not-exist` | 307 → `/enter`，与上面两条的落点不同，说明重定向确由 config 规则产生 |
| 本地实登跟到底 | dev 3200 + boss cookie，`curl -L` | 最终 200，落地页 h1 = "Sales Analysis" |
| 类型/构建 | `npx tsc --noEmit`、`npm run build` | 均通过 |

⚠️ 生产侧是未登录状态测的，只验证到「307 的目标地址对」；登录后落地页长什么样
没有在生产直接观测，靠的是同一份代码在本地实登验证通过。

**定性观测（用户/客户负责）**

在浏览器地址栏敲 https://www.johnstonebros.ie/classic/boss/reports/sales
（或点历史记录里的旧链接），确认它直接落到「销售分析」钻取页，
不再出现那个带度量/分组下拉的透视表页面。

问谁：用户本人。

**技术观测结果（20260920 部署后实测）**

| 项 | 结果 |
|---|---|
| 迁移应用 | ✅ `Order.invoiceNo`（text, nullable）+ `Order_invoiceNo_key` UNIQUE 索引都在；`_prisma_migrations` 里该条 `finished_at` 非空、未回滚 |
| 容器 | ✅ `app  Up About a minute (healthy)` |
| 部署后日志 | ✅ 近 10 分钟 error/exception 计数 **0**（⚠️ 日志被部署轮换，只覆盖本次之后） |
| 生产 URL | ✅ `/` 200 · `/enter` 200 · `/classic/boss/sales-analysis` 307（未登录跳登录，符合预期） |
| 未登录访问分析接口 | ✅ 401「未授权访问」，不是 500 |
| 新增 driver 维度 | ✅ 生产库实跑 30 天范围返回 8 个司机（BAO €4446.03、hanhua €1079.09…），unnest JOIN 可用 |
| 发号基线 | ✅ `Order.invoiceNo` 非空 **0 张**，干净起点；后续应只随销售单打印增长 |

**ℹ️ 一个会影响第一印象的事实**：生产销售数据**最新只到 2026-09-13**（7 单），
近 7 天 0 单。所以打开「按周」默认近 8 周，最近几周会是空的 —— 那不是功能坏了。
按月看 6/7/8 月数据才完整。

**⛔ 我没能验证的**：生产上没有可供我登录的账号，**新功能的真实渲染（按月开关、
维度面板、毛重/提成/单位三列）未经我直接验证**，只验到了状态码与接口层。
本地已用浏览器逐项实测过（见 docs/20260920-sales-analysis-multidim-tasks.md），
但生产渲染需要你本人确认。

**状态**：待观测（技术项已全部达成，定性项待用户确认）
