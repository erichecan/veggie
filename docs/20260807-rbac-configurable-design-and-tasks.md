...
- ~~**权限总览** — 角色 × 模块 只读全景矩阵~~ **已按用户要求去掉**（2026-08-07）。
  做出来了但用户看过后说不需要 —— 角色 tab 上每行已有权限点数，要看细节点进去就是。
...
- [x] **T11 权限中心 UI** ✅ 2026-08-07 · `0f13304`（+ 后续调整）
      扩 `/classic/operator/users` 为用户 / 角色 ~~/ 权限总览~~ tab，按 §6。
      **权限总览做完后按用户要求去掉了**，弹窗同时拉宽到视口宽度。
...
- [x] **T12 防锁死 + 审计 + 强制重登** ✅ 2026-08-07 · `0f13304`
      **验收**：
...
- [x] **T13 建 7 个业务角色（不自动分配）** ✅ 2026-08-07 · `0f13304`
...
| T11 | 2026-08-07 | `0f13304` | 权限中心三 tab；真浏览器逐个点过，控制台 0 error；顺手补上级列、多角色显示、预置角色中文名 |
| T12 | 2026-08-07 | `0f13304` | 防锁死三种走法全部 409 且事务回滚；改权限后旧 token 401 PERMISSION_CHANGED；**覆盖缺口见 §9.7** |
| T13 | 2026-08-07 | `0f13304` | 7 个业务模板；把 `purchase.order.approve` 从装饰变成真判定，并补回 BOSS/OPERATOR 免得审批静默中断 |
