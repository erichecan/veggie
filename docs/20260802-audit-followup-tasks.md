...
### D5 验证记录
- `npm run build` 通过，`ap-aging` 页面与 API 均进入构建产物
- 全量 120 个测试通过，tsc 通过
- 已部署（run 30769199788 success），**生产实测全绿**：
  应付账龄 BOSS=200（总额 0 / 草稿 25 张 €27,925.6，提示条数据正确）、无token=401、DRIVER=403；
  应收账龄仍 200、客户接口仍 401、健康检查 200——无回归

### 三项修复对审计判定的影响
重跑受影响探针后：**加权完成度 56.4% → 57.3%**
- M08-06 应收应付汇总表：partial → **done**（应付账龄补齐）
- M08-07 利润表：维持 missing，但缺口描述去掉「点进去 404 死链」（入口已摘）
- M11-04 自动备份：维持 partial，缺口从「异地留存绑在 GCS 上」改为
  「落点已可迁移，仍缺生产上接好持久落点」

artifact 已更新（同一链接），新增「核实之后动手修掉的三项」板块。

### D6 切 S3（进行中）
用户选定 **DigitalOcean Spaces / fra1 法兰克福**（与 Neon 生产库同区，且不新增司法区）。
- 已备 `scripts/verify-backup-store.ts`：部署前先验证凭据，写小对象→签URL取回逐字节比对→删除，
  只碰 `_verify/` 前缀。**避免用 14 分钟的构建去试 key 对不对**。
  该脚本自检时抓到自己一个 bug：`createReadStream` 惰性打开，只 destroy 不消费会在文件删除后
  抛出没人接的 error 事件——已改为读完再比对。
- 待用户提供：桶名 + 两个 Spaces 密钥。

## 遗留问题

- **生产备份仍未真正可用**：cloudbuild 现在显式写死 `BACKUP_DRIVER=gcs` 保持原状，
  但 GCS 桶依然不存在，备份仍会失败。要真正跑通需用户二选一：
  (a) 备一个 DigitalOcean Spaces 桶并配 `S3_*` 变量（推荐，与迁移方向一致）；
  (b) 建 GCS 桶（不推荐，迁移时还要再搬一次）。
  代码侧已就绪，只差落点。
- **恢复演练仍是人工**：合同要求「备份可正常恢复」，目前只有 runbook 命令，无自动验证。
- 上传文件（商品图片等）仍未纳入备份范围，只备数据库。
