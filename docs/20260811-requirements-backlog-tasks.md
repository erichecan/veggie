...
- 完成：4 / 56（Z1、Z3、Z8a、B3）
- 当前批次：**W1**（Z8b ⛔ 待用户拍板；B3 ✅ → 下一条 E3 库存守恒只读体检）
...
| **W1** | 只读走查 / 纯文档 / 纯前端，**不写库** | 13 | B3 ✅；余 8 条可做；A1/A2/B1/D2 四条走查 ⛔ 等 Z8b |
...
- [ ] **E3 库存守恒只读体检**
      验收：抽 20 个商品验证 库存数 == StockMove 累加；输出不守恒清单与规模
      依赖：无
      备注：已知存量 895 个商品不守恒，本任务只做量化不做清洗
