...
export default function CustomerProductsPage() {
  ...
  useEffect(() => {
    apiGet<{ products: Product[]; paymentTerm?: string }>('/api/customer-portal/products')
      ...
      .catch(() => toast.error(isEn ? 'Failed to load products' : '加载商品失败'))
  ...
  if (loading) return <div className="text-center py-20 text-gray-400">{isEn ? 'Loading products...' : '加载商品中...'}</div>
    ...
    <div className="space-y-4">
      ...
      {paymentTerm && (
        <p className="text-xs text-gray-500">
          {isEn ? 'Payment Term: ' : '结算方式：'}<span className="font-medium" style={{ color: PURPLE }}>{PAYMENT_LABELS[paymentTerm] ?? paymentTerm}</span>
        </p>
      )}
      ...
      {frequentProducts.length > 0 && (
        <div className="space-y-2">
          <h2 className="text-sm font-bold text-gray-700">🔁 {isEn ? 'Frequently Ordered' : '常购清单'}</h2>
      ...
      {filtered.length === 0 && !loading && (
        <div className="text-center py-16 text-gray-400">
          {search
            ? (isEn ? `No products found for "${search}"` : `没有找到 "${search}" 相关商品`)
