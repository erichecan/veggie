...
export default function ClassicBossLayout({ children }: { children: React.ReactNode }) {
  ...
  // 20260920：客户要求数据中心只留这几项，其余 15 个分析页（经营总览 / AI 问数 / 销售统计 /
  // 客户分析 / 毛利分析 / 采购进货分析 / 利润表 / 应收账龄 / 应付账龄 / 采购运营 / 物流分析 /
  // 司机提成 / 内控审计 / 销售分析（reports/sales）/ 物流分析（报表））只摘导航入口，
  // 页面与接口全部原样保留，直链仍可访问，要加回来就是把链接写回这个数组。
  const LINKS = [
