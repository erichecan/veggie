...
export default function SalesOverviewPage() {
  ...
  const SALES_EX_KEY = isEn ? 'Sales (Ex. Tax)' : '销售额税前'
  const SALES_INC_KEY = isEn ? 'Sales (Inc. Tax)' : '销售额税后'
  const AOV_KEY = isEn ? 'AOV' : '客单价'
  const SHORTAGE_KEY = isEn ? 'Shortage Rate' : '缺货率'

  const salesChartData = data.dailySeries.map((d) => ({
    day: String(d.date).slice(5, 10),
    [SALES_EX_KEY]: d.salesExTax,
    [SALES_INC_KEY]: d.salesIncTax,
  }))
  ...
  const shortageChartData = data.shortage.series.map((d) => ({
    ...
    [SHORTAGE_KEY]: Math.round(deriveShortageRate(d.shortage_lines, d.order_lines) * 1000) / 10,
  }))
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{isEn ? 'Sales Overview' : '销售统计'}</h1>
          <p className="text-sm text-gray-400 mt-0.5">{isEn ? 'Daily sales / AOV / shortage rate trends + top products · history reads daily snapshots' : '日销售额 / 客单价 / 缺货率趋势 + 关键商品排行 · 历史读每日快照'}</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="border rounded-lg p-4">
          <div className="text-xs text-gray-500">{isEn ? 'Period Sales (Ex. Tax)' : '期间销售额（税前）'}</div>
          <div className="text-2xl font-semibold mt-1 tabular-nums">{eur(totalSalesExTax)}</div>
        </div>
        <div className="border rounded-lg p-4">
          <div className="text-xs text-gray-500">{isEn ? 'Period Orders' : '期间订单数'}</div>
          <div className="text-2xl font-semibold mt-1 tabular-nums">{totalOrders}</div>
        </div>
        <div className="border rounded-lg p-4">
          <div className="text-xs text-gray-500">{isEn ? 'Average Order Value' : '平均客单价'}</div>
          <div className="text-2xl font-semibold mt-1 tabular-nums">{eur(avgAov)}</div>
        </div>
        <div className="border rounded-lg p-4">
          <div className="text-xs text-gray-500">{isEn ? 'Shortage Rate' : '缺货率'}</div>
          ...
          <div className="text-xs text-gray-400 mt-1">{isEn ? `${data.shortage.summary.shortageLines} / ${data.shortage.summary.orderLines} lines` : `${data.shortage.summary.shortageLines} / ${data.shortage.summary.orderLines} 行`}</div>
        </div>
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        <div className="border rounded-lg p-4 bg-white">
          <h2 className="text-sm font-medium text-gray-500 mb-3">{isEn ? 'Daily Sales Trend' : '日销售额趋势'}</h2>
          {salesChartData.length === 0 ? (
            <div className="text-center text-gray-400 py-10 text-sm">{isEn ? 'No data' : '暂无数据'}</div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={salesChartData} margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
                ...
                <Line type="monotone" dataKey={SALES_EX_KEY} stroke="#875A7B" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey={SALES_INC_KEY} stroke="#28a745" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="border rounded-lg p-4 bg-white">
          <h2 className="text-sm font-medium text-gray-500 mb-3">{isEn ? 'AOV Trend' : '客单价趋势'}</h2>
          {aovChartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={aovChartData} margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
                ...
                <Tooltip formatter={(v: unknown) => eur(Number(v))} />
                <Line type="monotone" dataKey={AOV_KEY} stroke="#875A7B" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="border rounded-lg p-4 bg-white">
          <h2 className="text-sm font-medium text-gray-500 mb-3">{isEn ? 'Shortage Rate Trend' : '缺货率趋势'}</h2>
          {shortageChartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={shortageChartData} margin={{ top: 5, right: 10, bottom: 0, left: 0 }}>
                ...
                <Line type="monotone" dataKey={SHORTAGE_KEY} stroke="#dc3545" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="border rounded-lg p-4 bg-white">
          <h2 className="text-sm font-medium text-gray-500 mb-3">{isEn ? 'Top 10 Products (by Sales)' : '关键商品 Top 10（按销售额）'}</h2>
          {data.topProducts.length === 0 ? (
            <div className="text-center text-gray-400 py-10 text-sm">{isEn ? 'No sales in this period' : '期内没有销售数据'}</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="text-left text-gray-400">
                <tr>
                  <th className="py-1 font-medium">#</th>
                  <th className="py-1 font-medium">{isEn ? 'Product' : '商品'}</th>
                  <th className="py-1 font-medium text-right">{isEn ? 'Qty' : '数量'}</th>
                  <th className="py-1 font-medium text-right">{isEn ? 'Sales' : '销售额'}</th>
                </tr>
              </thead>
              <tbody>
