...
export default function ArAgingPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">应收账龄</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="border rounded-lg p-4">
          ...
          <div className="text-xs text-gray-400 mt-1">{data.invoiceCount} 张未收清发票</div>
        </div>
        <div className="border rounded-lg p-4">
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <span className="text-xs text-gray-400">{filtered.length} 个欠款客户，按欠款额降序</span>
      </div>
        ...
        <table className="w-full text-sm min-w-[840px]">
          <thead className="bg-gray-50 text-left">
            <tr>
              ...
              <th className="px-3 py-2 font-medium text-right">发票数</th>
              <th className="px-3 py-2 font-medium text-right">最近还款</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((c) => (
              <tr key={c.customerId} className="border-t hover:bg-gray-50">
                ...
                <td className="px-3 py-1.5 text-right text-gray-500">
                  {c.lastPaidAt ? String(c.lastPaidAt).slice(0, 10) : '从未'}
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr><td colSpan={10} className="px-3 py-8 text-center text-gray-400">当前没有未收清的发票 👍</td></tr>
            )}
          </tbody>
        </table>
      </div>
