...
export default function SalesOrderDetailPage() {
  ...
  // 换客户(20260918)：下拉候选 + 编辑态当前选中的客户 id(order.restaurantId 是保存前的旧值)
  ...
  async function load() {
    ...
    try {
      ...
      // 换客户下拉的候选：slim=1 不带 specialPrices，选中后再按 id 取完整对象
      apiGet<Customer[]>('/api/customers?slim=1')
        .then(cs => setCustomers(cs.filter(c => c.isActive !== false)))
        .catch(() => {})
  ...
  // 换客户只在「还没进波次」时开放(与 dateLocked 同一集合,但两者是各自独立的业务判断,
  // 不共用一个变量)：进了波次就意味着拣货/配送已按原客户在排活，改抬头等于让司机把货送给另一个人。
