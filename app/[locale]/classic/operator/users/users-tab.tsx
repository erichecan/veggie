'use client'
/** 用户 tab：账号列表 + 新建 / 编辑 / 改密 / 停启用，以及每行的「权限」入口。 */
    ...
    >
      ...
      <td className="px-4 py-3 text-center">
        <div className="flex flex-wrap gap-1 justify-center">
          {allRoles.map(r => (
            <span key={r} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${ROLE_COLOR[r] ?? ROLE_COLOR.OTHER}`}>
              {roleLabel[r] ?? r}
            </span>
          ))}
        </div>
...
export default function UsersTab({
  ...
  async function handleSave() {
    ...
    try {
      ...
      // 改角色会连带改 UserRoleLink 与上级显示，整表重取比手工合并可靠
      onReload()
      setDialogOpen(false)
    } catch (e) {
  ...
  async function handleToggleActive(u: SystemUser) {
    try {
      await apiPut<SystemUser>(`/api/users/${u.id}`, { isActive: !u.isActive })
      toast.success(isEn
