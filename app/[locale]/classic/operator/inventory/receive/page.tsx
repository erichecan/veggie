...
export default function ReceivePage() {
  return (
    <Suspense fallback={<div className="py-16 text-center text-gray-400 text-sm">加载中...</div>}>
      <ReceivePageInner />
    </Suspense>
  )
}
...
function ReceivePageInner() {
  ...
  function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of files) {
      if (file.size > MAX_PHOTO_BYTES) {
        toast.error(`${file.name} 超过 5MB，请压缩后再上传`)
        continue
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : '收货失败')
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="text-lg font-semibold text-gray-800 mt-1">
              收货 · {selectedPo.name} <span className="text-sm font-normal text-gray-500">{supplierName}</span>
            </h2>
          </div>
        </div>
        ...
        {poLoading ? (
          ...
          <>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 grid grid-cols-3 gap-4">
              <div>
                <label className="block text-xs text-gray-500 mb-1">到货日期</label>
                <input type="date" value={arrivedAt} onChange={e => setArrivedAt(e.target.value)} className={`w-full ${inputCls}`} />
              </div>
              <div className="col-span-2">
                <label className="block text-xs text-gray-500 mb-1">来源核对备注（车辆/司机/供应商信息等）</label>
                <input type="text" value={notes} onChange={e => setNotes(e.target.value)}
                  placeholder="例如：白色厢式货车，司机张三，与供应商送货单一致"
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-gray-500 bg-gray-50 border-b border-gray-200">
                    <th className="text-left px-3 py-2.5">商品</th>
                    <th className="text-right px-3 py-2.5">订购</th>
                    <th className="text-right px-3 py-2.5">已收</th>
                    <th className="text-right px-3 py-2.5">本次良品</th>
                    <th className="text-right px-3 py-2.5">本次损坏</th>
                    <th className="text-left px-3 py-2.5">保质期</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={l.id} className="border-b border-gray-100">
                        <td className="px-3 py-2.5">
                          ...
                          <div className="text-xs text-gray-400">剩余待收 {remaining} {uomMap.get(l.uomId ?? '') ?? ''}</div>
                        </td>
              ...
              <div className="px-4 py-3 bg-gray-50 text-sm text-gray-600 flex items-center gap-4">
                <span>本次合计：良品 {totalGood}{totalBad > 0 && <span className="text-red-600"> · 损坏 {totalBad}</span>}</span>
                {totalBad > 0 && (
                  <span className="text-xs text-amber-600">损坏部分不入库，会记一笔报废留痕（{`到货即损坏（运输/供应商责任）`}），方便追损耗/找供应商索赔</span>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <label className="block text-xs text-gray-500 mb-2">取证照片（车辆/外包装/开箱点数/破损细节等，单张不超过 5MB）</label>
              <div className="flex items-center gap-3 flex-wrap">
                {photos.map((src, i) => (
                  <div key={i} className="relative">
                    <img src={src} alt={`收货照片 ${i + 1}`} className="w-20 h-20 rounded object-cover border border-gray-200" />
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-400">
          {viewMode === 'pending' ? `${pos.length} 张采购单待收货` : `共 ${history.length} 条收货记录`}
        </span>
      </div>
      ...
      {viewMode === 'pending' ? (
        listLoading ? (
          ...
          <div className="py-16 text-center text-gray-400 text-sm">暂无待收货采购单</div>
        ) : (
              ...
              return (
                <button key={po.id} onClick={() => openPo(po.id)}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-gray-400">预计到货 {formatDateOnly(po.expectedDate) || '—'}</span>
                  </div>
        ...
        <div className="py-16 text-center text-gray-400 text-sm">暂无收货记录</div>
      ) : (
              ...
              <div key={gr.id} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <button
                  ...
                  <div>
                    ...
                    {badLines.length > 0 && (
                      <span className="text-xs text-red-600 ml-2">{badLines.length} 项有损坏</span>
                    )}
                  </div>
                  <div className="flex items-center gap-3 text-xs text-gray-400">
                    <span>到货 {formatDateOnly(gr.arrivedAt)}{gr.receivedBy ? ` · ${gr.receivedBy}` : ''}</span>
                ...
                {expanded && (
                  <div className="px-4 pb-4 border-t border-gray-100 pt-3">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-xs text-gray-500">
                          ...
                          <th className="text-right py-1">良品数量</th>
                          <th className="text-right py-1">损坏数量</th>
                        </tr>
                      </thead>
                      <tbody>
                        {gr.lines.map((l, i) => {
                      ...
                      {(goodLines.length > 0 || badLines.length > 0) && (
                        <tfoot>
                          <tr className="border-t border-gray-200 font-medium">
                            <td className="py-1.5 text-gray-600">合计</td>
                    ...
                    {gr.notes && <div className="text-xs text-gray-500 mt-2">备注：{gr.notes}</div>}
                    {(gr.photos?.length ?? 0) > 0 && (
                      <div className="flex gap-1.5 mt-2 flex-wrap">
                        {(gr.photos ?? []).map((src, i) => (
                          <img key={i} src={src} alt={`${gr.name} 照片 ${i + 1}`} className="w-16 h-16 rounded object-cover border border-gray-200" />
                        ))}
                      </div>
                    )}
                  </div>
