  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400">
        ...
        <span>/</span>
        <span style={{ color: PURPLE }}>损耗与退货仪表盘</span>
      </div>

      <div>
        <h1 className="text-lg font-semibold" style={{ color: PURPLE }}>损耗与退货仪表盘</h1>
        <p className="text-sm text-gray-500 mt-0.5">近 {DAYS} 天仓库报废损耗 + 客户退货去向，一张页面看清货去哪了</p>
      </div>
      ...
      <div className="bg-white rounded border grid grid-cols-3 divide-x" style={{ borderColor: BORDER }}>
        <div className="p-4">
          <div className="text-xs text-gray-500 flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: AMBER }} />本期报废损耗值
          </div>
          <div className="text-lg font-bold" style={{ color: AMBER }}>{eur(kpis?.scrapValueThisPeriod ?? 0)}</div>
          <div className="text-xs text-gray-400 mt-0.5">近 {DAYS} 天，含仓库报废与客退报废</div>
        </div>
        <div className="p-4">
          <div className="text-xs text-gray-500 flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: GREEN }} />退货回收率
          </div>
          <div className="text-lg font-bold" style={{ color: kpis?.returnRecoveryRatePct == null ? GRAY : GREEN }}>
            {kpis?.returnRecoveryRatePct == null ? '暂无数据' : `${kpis.returnRecoveryRatePct}%`}
          </div>
          <div className="text-xs text-gray-400 mt-0.5">已审批退货中回补可售库存的占比</div>
        </div>
        <div className="p-4">
          <div className="text-xs text-gray-500 flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full" style={{ background: RED }} />待处置退货
          </div>
          <div className="text-lg font-bold" style={{ color: (kpis?.pendingReturnDisposition ?? 0) > 0 ? RED : GRAY }}>
            {kpis?.pendingReturnDisposition ?? 0}
          </div>
          <div className="text-xs text-gray-400 mt-0.5">当前队列深度，不限时间范围</div>
        </div>
      </div>

      {/* 退货去向 split bar */}
      <div className="bg-white rounded border p-4" style={{ borderColor: BORDER }}>
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">退货去向（近 {DAYS} 天）</h2>
        {split && (split.sellableQty + split.scrapQty + split.pendingQty) > 0 ? (
          <>
            <div className="flex h-6 rounded overflow-hidden border" style={{ borderColor: BORDER }}>
              {split.sellablePct > 0 && (
                <div style={{ width: `${split.sellablePct}%`, background: GREEN }} title={`可再售 ${split.sellableQty}`} />
              )}
              {split.scrapPct > 0 && (
                <div style={{ width: `${split.scrapPct}%`, background: AMBER }} title={`报废 ${split.scrapQty}`} />
              )}
              {split.pendingPct > 0 && (
                <div style={{ width: `${split.pendingPct}%`, background: GRAY }} title={`待处置 ${split.pendingQty}`} />
              )}
            </div>
            <div className="flex flex-wrap gap-x-6 gap-y-1.5 mt-3 text-xs">
              <span className="flex items-center gap-1.5">
                ...
                可再售 {split.sellableQty} ({split.sellablePct}%)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-sm" style={{ background: AMBER }} />
                报废 {split.scrapQty} ({split.scrapPct}%)
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-sm" style={{ background: GRAY }} />
                待处置 {split.pendingQty} ({split.pendingPct}%)
              </span>
            </div>
          </>
        ) : (
          <div className="py-6 text-center text-sm text-gray-400">暂无退货数据</div>
        )}
      </div>

      {/* Weekly trend */}
      <div className="bg-white rounded border p-4" style={{ borderColor: BORDER }}>
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">近 {WEEKS} 周报废趋势（数量）</h2>
        {trend.length === 0 || trend.every(w => w.scrapQty === 0) ? (
          <div className="py-6 text-center text-sm text-gray-400">暂无报废记录</div>
        ) : (
              ...
              return (
                <div key={w.weekLabel} className="flex-1 flex flex-col items-center justify-end h-full">
                  <div className="w-full flex flex-col justify-end rounded-sm overflow-hidden" style={{ height: `${totalH}%`, minHeight: 2, background: '#f0e4ee' }}>
                    <div style={{ height: `${returnH}%`, background: AMBER }} title={`客退报废 ${w.returnScrapQty}`} />
                    <div style={{ flex: 1, background: PURPLE }} title={`其他报废 ${round1(w.scrapQty - w.returnScrapQty)}`} />
                  </div>
        ...
        <div className="flex gap-4 mt-3 text-xs text-gray-500">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: PURPLE }} />仓库报废</span>
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: AMBER }} />客退报废</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        {/* Reason breakdown */}
        <div className="bg-white rounded border p-4" style={{ borderColor: BORDER }}>
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">报废原因排行（近 {DAYS} 天）</h2>
          {reasons.length === 0 ? (
            <div className="py-6 text-center text-sm text-gray-400">暂无报废记录</div>
          ) : (
            <div className="space-y-2">
              {reasons.map(r => (
                <div key={r.reason} className="flex items-center gap-2">
                  <span className="text-xs text-gray-700 w-24 truncate shrink-0">{r.reasonLabel}</span>
        ...
        <div className="bg-white rounded border overflow-hidden" style={{ borderColor: BORDER }}>
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 pt-4 mb-1">损耗 TOP 商品（近 {DAYS} 天）</h2>
          {topLoss.length === 0 ? (
            <div className="py-6 text-center text-sm text-gray-400">暂无报废记录</div>
          ) : (
            <div>
              ...
              >
                <span>商品</span><span className="text-right">数量</span><span className="text-right">损耗值</span>
              </div>
              {topLoss.map(item => (
                <div
                  key={item.productId}
