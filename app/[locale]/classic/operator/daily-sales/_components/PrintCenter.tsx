...
function BatchCard({
  ...
  // 20260907 推翻 20260906 的"解锁=取消整波司机安排"：解锁只清拣货锁，订单仍留在原司机身上，
  // 可以继续编辑；要撤销司机安排请用调度台的改派/移出待分配，而不是这个按钮。
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {pickLockedAt && canUnlock && (
            <button
              ...
              title={isEn ? 'Unlock this batch for picking — driver assignment stays unchanged' : '解开该批次的拣货锁——司机安排保持不变'}
            >
              {isEn ? '🔒 Unlock' : '🔒 解锁'}
            </button>
          )}
