...
const HELP_CONTENT_ZH: HelpRole[] = [
  {
    ...
    sections: [
      ...
      {
        id: 'op-rbac',
        title: '角色与权限管理（谁能看什么、能做什么）',
        emoji: '🔐',
        articles: [
          {
            id: 'op-rbac-concept',
            title: '先搞清三个概念：权限点 / 角色 / 数据范围',
            steps: [
              '权限点 = 一个具体动作，比如「确认订单」「查看价格表」。系统一共 181 个，按 12 个模块分组（销售 21 个、采购 14 个、库存 20 个、配送 23 个、财务 25 个、基础档案 38 个……）。权限点由开发定义，界面上只能勾选、不能新增。',
              '角色 = 一组权限点的集合，比如「打印中心」这个角色 = 只打单：拣货单、配送单、发票的打印视图。角色可以自己建。',
              '数据范围 = 这个角色能看到哪些记录，三选一：「全部数据」看全公司；「本人及下属」按上级关系向下展开；「仅本人」只看自己名下的。它与权限点是两个维度——权限点管「能不能做这个动作」，数据范围管「能对谁的数据做」。',
              '一个用户可以有多个角色，权限取并集（有任一角色给了就有），数据范围取最宽的那个。',
            ],
            note: '带 🔒 的是预置角色（由系统平迁自动生成），不能删除，但权限可以改。自己新建的角色可以随便删。',
          },
          {
            id: 'op-rbac-create',
            title: '如何新建一个角色并授权（完整步骤）',
            steps: [
              '进入顶部导航「系统 → 用户管理」，点上方的「🎭 角色」标签页。',
              '点右上角「新建角色」。',
              '填「名称」（给人看的，如 办公室销售）和「标识」（英文小写下划线，如 office_sales，用于系统内部识别，建好后别改）。',
              '「说明」填这个角色是干什么的——列表页会显示，日后别人接手时全靠这句话判断该不该用它。',
              '选「数据范围」：不确定就先选「仅本人」，宁可给窄了再放开，不要一上来给「全部数据」。',
              '在「权限点」区域按模块勾选。上方有筛选框可以按模块名搜；右上角显示「已选 N / 181」，也有「全选 / 清空」。',
              '点「保存」。回到列表就能看到这个角色，以及它的权限点数和在用人数。',
            ],
            tip: '更省事的办法：找一个最接近的现有角色，点它那行的「复制」，改个名字再微调。比从 0 勾 181 个权限点快得多，也不容易漏。',
          },
          {
            id: 'op-rbac-assign',
            title: '把角色分配给用户',
            steps: [
              '切回「👤 用户」标签页，找到要改的人，点他那行的「权限」。',
              '勾选要给他的角色（可多选），保存。',
              '列表的「角色」列会显示这个人当前的全部角色标签。',
            ],
            note: '⚠️ 权限变更后该用户必须重新登录才生效——系统会强制他退出。改完记得通知本人，否则他会以为系统坏了。',
          },
          {
            id: 'op-rbac-verify',
            title: '改完一定要验证（最容易被跳过的一步）',
            steps: [
              '用那个人的账号实际登录一次，点开他该用的页面，确认能进；再点一个他不该有的页面，确认进不去。',
              '权限「给多了」不会报错，只会悄悄放行——所以只看配置页勾了什么是验不出问题的，必须实际点。',
              '不确定某个角色现在能做什么，看角色列表里的「权限点」数字，点进去逐模块看勾选状态。',
            ],
            note: '⚠️ 已知问题：181 个权限点里有 13 个目前**没有任何判定会读取**（如「确认订单」「取消订单」「调度台访问」等）。勾不勾它们都不影响实际行为——这类开关比没有更危险，因为它让人以为限制住了。开发侧已记录，等待清理；在此之前，涉及这几个动作的限制不要只靠勾选，要实际验证。',
          },
        ],
      },
      {
        id: 'op-uom',
        title: '计量单位（UoM）设置与管理',
        emoji: '📏',
        articles: [
          {
            id: 'op-uom-concept',
            title: '计量单位是什么：三层结构一次讲清',
            steps: [
              '计量单位分三层：① 单位分类（如"重量"、"计件"）② 该分类下的具体单位（如 kg、g、箱）③ 每个单位的换算系数 Factor（1 个本单位 = factor 个基准单位）。',
              '每个分类必须有且只有一个"基准单位"（Factor=1），其余单位相对它换算：Factor>1 是比基准更大的单位（类型自动显示"大单位"），Factor<1 是比基准更小的单位（类型自动显示"小单位"）。',
              '举例（重量分类，基准单位=kg）：g 的 Factor=0.001（小单位），10kg BAG 的 Factor=10（大单位）。下单填 3 个「10kg BAG」，换算成基准单位 = 3 × 10 = 30kg。',
              '举例（计件分类，基准单位=PCS/件）：DOZEN（打）Factor=12，CASE（箱）Factor=12。下单填 2 DOZEN，换算成 PCS = 2 × 12 = 24 件。',
            ],
            note: '"类型"（基准 / 大单位 / 小单位）不用手动选，系统根据 Factor 自动判断：Factor=1 → 基准，Factor>1 → 大单位，Factor<1 → 小单位。',
          },
          {
            id: 'op-uom-create',
            title: '如何新建 / 管理计量单位',
            steps: [
              ...
              '列表每一行能看到该单位的 Factor、精度、自动判定的类型（基准 / 大单位 / 小单位），以及货物类型和拣货明细（都可在列表里直接改，行内下拉即改即生效）。',
              '「货物类型」决定这个货出现在哪张拣货单（大货进整箱整袋单、散货进零散货单）；「拣货明细」决定那张单上要不要逐个客户列出来——配货时现切现称、每份贴客户标签的选「按客户展开」（如冬瓜按 KG 卖，一家 2.5kg、一家 6.2kg），提前备好按包数拿的用「只显示总量」（如 1KG 一包就是 1kg）。两者互不影响，写了备注的行则永远会列出该客户。',
              '不需要的单位点「停用」即可，不是物理删除——历史订单、商品上已经用过这个单位的数据保留不受影响，只是新建 / 编辑时的下拉菜单里不再出现。',
            ],
            tip: '同一个分类里新建单位前先想清楚 Factor 数值——比如已有 kg（Factor=1）、g（Factor=0.001），要加"两公斤袋"就填 Factor=2，不要拿 kg 数值反推错了倍数。',
          },
          {
            id: 'op-uom-product-link',
            title: '给商品关联计量单位（销售单位 / 采购单位）',
            steps: [
              '进入「商品管理」页面，点击要设置的商品进入详情，点「编辑」。',
              '在「Inventory & UoM」区块，「Unit of Measure」下拉选的是这个商品对客户销售时用的单位；「Purchase UoM」下拉选的是向供应商采购时用的单位——两个可以选不同单位，比如采购按「箱 CASE」进货、销售按「kg」拆零卖。',
              '两个下拉框列出的是全部已建好的计量单位（不按分类过滤），系统不会自动检查你选的两个单位是不是同一分类、换算是否合理，需要自己确认这两个单位之间的关系（分类不一样的话，系统更是完全不做任何自动换算，采购数量和销售数量各自独立记录，互不影响）。',
              '保存后，这个商品在下单 / 报价页面显示的默认单位就是这里设置的「Unit of Measure」。',
            ],
            note: '同一个商品同时间只能设一个销售单位——这一点直接关系到下一条「小单位怎么处理」，建议先看完下一条再决定怎么建商品。',
          },
          {
            id: 'op-uom-loose-flex',
            title: '同一件货想按不同的小单位灵活卖给不同客户，现在怎么处理？（举例：橙子按箱走 vs 单独给某家配几颗）',
            steps: [
              '先说清楚现状：目前系统里一个商品在任意时刻只能设一个「Unit of Measure（销售单位）」，下单 / 报价时这一行订单会直接复制商品当前设置的销售单位，操作员在下单页面没有办法临时把某一行换算成另一个（更小的）单位来卖。',
              '也就是说"这个商品平时整箱走货给大部分客户，但有一家客户只要散称几颗 / 几斤"这种场景，系统现在没有自动换算或临时改单位的功能，需要靠人工用以下方法之一处理。',
              '方法一（推荐，最常用）：为同一款货物建两条独立的商品记录，比如「橙子（箱装）」销售单位设为 CASE，「橙子（散装 / 每颗）」销售单位设为 PCS，两者各自独立定价、独立下单，互不影响彼此的库存和订单历史。要给某家客户下"10 颗橙子"的单，就直接在下单页面找到「橙子（散装）」这个商品，数量填 10。',
              '方法二（临时 / 一次性场景）：仍用同一个商品下单，在订单行的「规格 Spec」字段里手写备注说明实际按什么单位、给多少（如"散装10颗，非整箱"），但这只是给拣货 / 配送人员看的文字提醒，系统不会按这段文字自动拆分数量或改价格，金额还是按订单行原来的单位和单价计算，需要操作员自己在下单时把单价和数量填对。',
              '货物类型「散货 LOOSE」不能解决这个问题：它只影响拣货单打印时"要不要把这个商品的汇总数量拆开显示每个客户各要几个"，跟"能不能换算成别的小单位卖"是两回事，不要混在一起。',
            ],
            note: '如果这种"一个商品需要按多种计量单位灵活出单、系统自动换算价格和数量"的场景以后会经常发生，这属于系统当前不支持的功能缺口，建议整理具体需求（哪些商品、需要哪几种单位、换算价格怎么算）后单独提出来评估开发，不建议长期只靠方法一 / 方法二人工顶。',
            tip: '判断该不该拆成两个商品的经验法则：如果这个"小单位版本"会被固定回头下单（不是一次性），就走方法一新建商品，方便以后直接选；只是偶尔一次性特殊要求，才用方法二写备注顶一下。',
          },
          {
            id: 'op-uom-goodstype',
            title: '货物类型（大货 BULK / 散货 LOOSE）到底影响什么',
            steps: [
              '「货物类型」是计量单位（Uom）上的一个字段，不是商品上的字段——同一个商品用的销售单位是大货还是散货，就决定了这个商品在拣货单里的呈现方式。',
              '大货 BULK：拣货单上这个商品只显示一行汇总数量（比如"苹果 30kg"），不展开列每个客户具体要几个。',
              '散货 LOOSE：拣货单上这个商品会展开显示每个客户各要多少（比如"苹果：A 餐馆 5kg，B 餐馆 3kg，C 餐馆 2kg"），方便拣货员按客户单独分装。',
              '这个字段只影响打印展示，不影响下单流程、不影响价格计算，也不会自动帮你把订单拆成小单位卖。',
            ],
            note: '如果发现某个商品的拣货单没有按客户展开明细，先去「设置 → 计量单位」检查这个商品用的销售单位有没有设成「散货 LOOSE」。',
          },
        ],
      },
      {
...
const HELP_CONTENT_EN: HelpRole[] = [
  {
    ...
    sections: [
      ...
      {
        id: 'op-uom',
        title: 'Units of Measure (UoM) Setup & Management',
        emoji: '📏',
        articles: [
          {
            id: 'op-uom-concept',
            title: 'What Is a Unit of Measure: The Three-Layer Structure',
            steps: [
              'Units of measure have three layers: ① a category (e.g. "Weight", "Count") ② the specific units under that category (e.g. kg, g, box) ③ each unit\'s conversion Factor (1 unit = factor × reference units).',
              'Every category must have exactly one "reference unit" (Factor = 1). All other units convert relative to it: Factor > 1 is a bigger unit than the reference (type auto-shows "Bigger"), Factor < 1 is a smaller unit (type auto-shows "Smaller").',
              'Example (Weight category, reference = kg): g has Factor = 0.001 (Smaller), 10kg BAG has Factor = 10 (Bigger). Entering 3 "10kg BAG" on an order converts to 3 × 10 = 30kg in the reference unit.',
              'Example (Count category, reference = PCS): DOZEN has Factor = 12, CASE has Factor = 12. Entering 2 DOZEN on an order converts to 2 × 12 = 24 PCS.',
            ],
            note: 'The "type" (Reference / Bigger / Smaller) is never picked manually — the system derives it from Factor automatically: Factor = 1 → Reference, Factor > 1 → Bigger, Factor < 1 → Smaller.',
          },
          {
            id: 'op-uom-create',
            title: 'How to Create / Manage Units of Measure',
            steps: [
              ...
              'Each row in the list shows that unit\'s Factor, Rounding, auto-derived Type (Reference / Bigger / Smaller), plus Goods Type and Picking Detail (both editable inline — the dropdown in the row saves immediately).',
              '"Goods Type" decides which picking sheet the item lands on (Bulk → full case/bag sheet, Loose → loose-goods sheet). "Picking Detail" decides whether that sheet lists each customer separately — choose "By customer" for goods cut or weighed per order (e.g. winter melon by KG, one customer wants 2.5kg and another 6.2kg), keep "Total only" for pre-packed units the picker just counts (e.g. 1KG). The two are independent, and a line with a note always shows that customer either way.',
            ...
            tip: 'Before creating a new unit in an existing category, work out the Factor value first — e.g. if kg (Factor=1) and g (Factor=0.001) already exist, a "2kg bag" should be Factor=2. Don\'t back-derive the multiplier from the kg value and get it wrong.',
          },
          {
            id: 'op-uom-product-link',
            title: 'Linking a Product to Its Units of Measure (Sales UoM / Purchase UoM)',
            steps: [
              'Go to "Product Management", click the product you want to configure to open its detail page, then click "Edit".',
              'In the "Inventory & UoM" block, the "Unit of Measure" dropdown is the unit used when selling to customers; "Purchase UoM" is the unit used when buying from suppliers — the two can be different, e.g. purchase by "CASE" but sell broken down by "kg".',
              'Both dropdowns list every active unit of measure (not filtered by category) — the system does not check whether the two units you picked belong to the same category or whether the conversion makes sense. You need to confirm that relationship yourself (and if the categories differ entirely, the system does zero automatic conversion — purchase quantity and sales quantity are recorded completely independently).',
              'After saving, the default unit shown for this product on the ordering / quotation pages is whatever you set as "Unit of Measure" here.',
            ],
            note: 'A product can only have one Sales UoM at any given time — this directly relates to the next article on handling smaller units. Read that one before deciding how to set up a product.',
          },
          {
            id: 'op-uom-loose-flex',
            title: 'Selling the Same Item in a Different Small Unit for a Specific Customer — How Is This Handled Today? (Example: oranges by the case vs. a handful of pieces for one restaurant)',
            steps: [
              'Current state, plainly: right now a product can only have one "Unit of Measure (Sales UoM)" at a time. When you add it to an order/quotation, that line simply copies the product\'s currently configured sales unit — there\'s no way in the ordering screen to temporarily convert a single line to a different (smaller) unit.',
              'In other words, the scenario of "this product normally ships by the full case to most customers, but one restaurant only wants a handful loose by weight or count" has no built-in automatic conversion or per-line unit picker today. It has to be handled manually with one of the two methods below.',
              'Method 1 (recommended, most common): create two separate product records for the same item — e.g. "Oranges (Case)" with Sales UoM set to CASE, and "Oranges (Loose/Each)" with Sales UoM set to PCS. Each is priced and ordered independently, with its own stock and order history. To sell "10 oranges" to a specific customer, just find "Oranges (Loose)" on the ordering page and enter quantity 10.',
              'Method 2 (for one-off / occasional cases): keep using the same product, and write a manual note in the order line\'s "Spec" field describing what\'s actually being given (e.g. "loose, 10 pieces, not a full case"). This is only a text reminder for pickers/drivers — the system does not parse it to split quantities or adjust pricing automatically. The amount charged still follows the line\'s original unit and unit price, so the operator has to enter the correct quantity and price by hand.',
              'The "Loose" goods type does NOT solve this: it only affects whether the picking list breaks a product\'s total quantity down per customer when printed — that\'s a separate concept from "can this line be sold in a different smaller unit." Don\'t conflate the two.',
            ],
            note: 'If the need for "one product sold flexibly in multiple units with the system auto-converting quantity and price" comes up regularly, that\'s a real feature gap in the current system — worth writing up as a proper requirement (which products, which units, how the price should convert) rather than relying on Method 1/2 as a permanent workaround.',
            tip: 'Rule of thumb for whether to split into two products: if the "small-unit version" will be ordered repeatedly going forward (not a one-off), use Method 1 and create a real product so it\'s easy to select next time; if it\'s a rare, one-off special request, Method 2\'s manual note is enough.',
          },
          {
            id: 'op-uom-goodstype',
            title: 'What Does Goods Type (Bulk / Loose) Actually Affect',
            steps: [
              '"Goods Type" is a field on the Unit of Measure (Uom) itself, not on the product — whichever sales unit a product uses determines how that product is displayed on the picking list.',
              'Bulk: the picking list shows a single summed line for this product (e.g. "Apples 30kg") without breaking it down by which customer wants how much.',
              'Loose: the picking list expands this product to show exactly how much each customer wants (e.g. "Apples: Restaurant A 5kg, Restaurant B 3kg, Restaurant C 2kg"), so pickers know to portion it out per customer.',
              'This field only affects print/display grouping — it does not affect the ordering flow, does not affect pricing, and does not automatically split an order into a smaller unit for you.',
            ],
            note: 'If a product\'s picking list isn\'t breaking out per-customer detail the way you expect, check "Settings → Units of Measure" first to see whether that product\'s sales unit is set to "Loose".',
          },
        ],
      },
      {
