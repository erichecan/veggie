    ...
    <div className="bg-white rounded-lg border" style={{ borderColor: '#e5e7eb' }}>
      ...
      <div className="px-4 py-4 border-b space-y-3" style={{ borderColor: '#e5e7eb' }}>
        ...
        <div className="flex items-start gap-2">
          <span className={labelCls}>司机</span>
          <div className="flex flex-wrap gap-1.5">
            <Chip active={driverSel.size === 0} onClick={() => setDriverSel(new Set())}>全部司机</Chip>
            {driverOpts.length === 0 && <span className="text-xs text-gray-300 pt-1">无可选项</span>}
            {driverOpts.map(d => (
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            <Chip active={slotSel === ''} onClick={() => setSlotSel('')}>全部时段</Chip>
            <Chip active={slotSel === 'am'} onClick={() => setSlotSel('am')}>上午</Chip>
            <Chip active={slotSel === 'pm'} onClick={() => setSlotSel('pm')}>下午</Chip>
          </div>
        </div>

        {/* 批次 */}
        <div className="flex items-start gap-2">
          <span className={labelCls}>批次</span>
          <div className="flex flex-wrap gap-1.5">
            <Chip active={batchSel.size === 0} onClick={() => setBatchSel(new Set())}>全部批次</Chip>
            {batchOpts.length === 0 && <span className="text-xs text-gray-300 pt-1">无可选项</span>}
            {batchOpts.map(b => (
              <Chip key={b} active={batchSel.has(b)} onClick={() => toggleNum(batchSel, b, setBatchSel)}>批次 {b}</Chip>
            ))}
          </div>
        </div>
        ...
        {/* 按星期筛选 */}
        <div className="flex items-start gap-2">
          <span className={labelCls}>按星期筛选</span>
          <div className="flex flex-wrap gap-1.5">
            {WEEKDAYS.map((w, i) => (
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b flex-wrap gap-2" style={{ borderColor: '#e5e7eb', background: '#faf5fb' }}>
        <div className="flex items-center gap-1.5">
          ...
          <Chip active={sortBy === 'batch'} onClick={() => setSortBy('batch')}>批次</Chip>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-gray-500" style={{ background: '#faf5fb' }}>
              ...
              <th className="text-left font-semibold px-3 py-2.5">日期</th>
          ...
          <tbody>
            ...
            {!loading && sorted.length === 0 && (
              <tr><td colSpan={9} className="text-center py-10 text-gray-400">该筛选条件下暂无批次数据</td></tr>
            )}
          ...
          {sorted.length > 0 && (
            <tfoot>
              <tr className="border-t font-semibold" style={{ borderColor: '#e5e7eb', background: '#f9fafb' }}>
                <td className="px-3 py-2.5" colSpan={4}>合计 {driverCount} 名司机 · {sorted.length} 趟</td>
