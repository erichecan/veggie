...
export default function FreshDailySuggestionsPage() {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      toast.success(isEn ? `Generated ${res.generated} restock suggestion(s)` : `已生成 ${res.generated} 条备货建议`)
      await load()
    } catch (e) {
  ...
  async function handleConvert() {
    if (selected.size === 0) { toast.error(isEn ? 'Please select items to purchase first' : '请先勾选要采购的商品'); return }
    setConverting(true)
    try {
      ...
      if (res.skippedCount > 0) {
        toast.warning(isEn
          ? `${res.skippedCount} item(s) skipped (no supplier configured), no PO created`
          : `${res.skippedCount} 项因无配置供应商被跳过，未生成采购单`)
      }
      if (res.createdPOs.length === 1) {
        toast.success(isEn ? `Purchase order ${res.createdPOs[0].name} created` : `已生成采购单 ${res.createdPOs[0].name}`)
        router.push(`${prefix}/classic/operator/purchases/${res.createdPOs[0].id}`)
      } else if (res.createdPOs.length > 1) {
        toast.success(isEn
          ? `Split into ${res.createdPOs.length} purchase orders by supplier: ${res.createdPOs.map(p => p.name).join(', ')}`
          : `已按供应商拆分生成 ${res.createdPOs.length} 张采购单：${res.createdPOs.map(p => p.name).join(', ')}`)
        router.push(`${prefix}/classic/operator/purchases`)
      }
      await load()
    } catch (e) {
      toast.error(e instanceof Error ? e.message : (isEn ? 'Failed to create purchase order' : '生成采购单失败'))
    } finally {
      setConverting(false)
    }
  }
    ...
    <div className="flex flex-col h-full overflow-auto bg-gray-50">
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="text-xs text-gray-500 mb-1">
          <button onClick={() => router.push(`${prefix}/classic/operator/purchases`)} className="hover:underline" style={{ color: PURPLE }}>{isEn ? 'Purchasing' : '采购'}</button>
          <span className="mx-1 text-gray-400">/</span>
          <span className="text-gray-700 font-medium">{isEn ? 'Fresh & Frozen · Next-day Restock List' : '生鲜冷冻 · 次日备货清单'}</span>
        </div>
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h1 className="text-xl font-bold text-gray-900">{isEn ? '🥬 Fresh & Frozen Produce · Next-day Restock List' : '🥬 蔬菜生鲜冷冻 · 次日备货清单'}</h1>
          <button
          ...
          >
            {generating ? (isEn ? 'Generating…' : '生成中…') : generatedToday ? (isEn ? 'Regenerate' : '重新生成') : (isEn ? "Generate today's suggestions" : '生成今日建议')}
          </button>
        </div>
      </div>
      ...
      <div className="p-6">
        <div className="mb-4 flex items-center gap-2 px-4 py-2.5 rounded text-sm" style={{ background: '#e5f1e9', color: '#2e7d4f' }}>
          {isEn
            ? '✓ Based on the last 3 days’ average daily outbound + confirmed future orders, ignoring safety stock (fresh produce is not stockpiled). '
            : '✓ 依据近 3 日日均出货 + 已确认未来订单，不看安全库存（生鲜不囤货）。'}
          {isEn
            ? (generatedToday ? "Today's suggestions have been generated." : "Today's suggestions have not been generated yet. Click \"Generate today's suggestions\" above.")
        ...
        <div className="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center text-gray-400 text-sm">
              {isEn
                ? (generatedToday ? 'No fresh produce needs restocking today — covered by stock and confirmed orders.' : 'Click "Generate today\'s suggestions" above to start')
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr style={{ background: '#f8f8f8', borderBottom: '1px solid #e8e8e8' }}>
                    ...
                    <th className="px-4 py-2.5 text-right font-medium text-gray-600 text-xs">{isEn ? 'Avg Daily Outbound (3d)' : '近3日日均出货'}</th>
                    <th className="px-4 py-2.5 text-right font-medium text-gray-600 text-xs">{isEn ? 'Confirmed Future Orders' : '已确认未来订单'}</th>
                    <th className="px-4 py-2.5 text-right font-medium text-gray-600 text-xs">{isEn ? 'Current Stock' : '现有库存'}</th>
                    <th className="px-4 py-2.5 text-right font-medium text-gray-600 text-xs">{isEn ? 'In-Transit Purchases' : '在途采购'}</th>
                    <th className="px-4 py-2.5 text-right font-medium text-gray-600 text-xs">{isEn ? 'Suggested Qty' : '建议采购量'}</th>
                    <th className="px-4 py-2.5 text-left font-medium text-gray-600 text-xs">{isEn ? 'Unit' : '单位'}</th>
                    <th className="px-4 py-2.5 text-left font-medium text-gray-600 text-xs">{isEn ? 'Suggested Supplier' : '建议供应商'}</th>
                    <th className="px-4 py-2.5 text-right font-medium text-gray-600 text-xs">{isEn ? 'Estimated Cost' : '预估成本'}</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map(r => (
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50">
                <div className="text-sm text-gray-600">
                  {isEn ? (
                    <>Selected <b className="text-gray-900">{selected.size}</b> item(s) · Est. total <b className="text-gray-900">{eur(selectedCost)}</b></>
                  ) : (
                ...
                >
                  {converting ? (isEn ? 'Generating…' : '生成中…') : (isEn ? 'Create Purchase Order' : '生成采购单')}
                </button>
              </div>
            </div>
          )}
