...
const STATUS_LABEL_ZH: Record<StockTake['status'], { text: string; cls: string }> = {
...
const STATUS_LABEL_EN: Record<StockTake['status'], { text: string; cls: string }> = {
  DRAFT: { text: 'Counting', cls: 'bg-amber-100 text-amber-800' },
  DONE: { text: 'Completed', cls: 'bg-green-100 text-green-800' },
...
export default function StockTakePage() {
  ...
  async function handleCreate() {
    if (creating) return
    if (selectedCats.size === 0) { toast.error(isEn ? 'Please select at least one category' : '请至少选择一个分类'); return }
    setCreating(true)
    try {
      ...
      toast.success(isEn
        ? `Stock take ${created.name} created (${created.lines?.length ?? 0} products)`
        : `盘点单 ${created.name} 已创建（${created.lines?.length ?? 0} 个商品）`)
      setCreateOpen(false)
  ...
  async function saveCounts(silent = false) {
    ...
    try {
      ...
      if (bad) { toast.error(isEn ? 'Counted quantity must be a number ≥ 0' : '实盘数量必须是 ≥ 0 的数字'); return false }
      const updated = await apiPatch<StockTake>(`/api/stock-takes/${detail.id}`, {
        action: 'save_counts', counts: payload,
      })
      setDetail(updated)
      if (!silent) toast.success(isEn ? 'Saved' : '已保存')
      return true
    } catch (e) {
      toast.error((e as Error).message)
      return false
  ...
  async function complete() {
    ...
    if (filled === 0) { toast.error(isEn ? 'No counted quantities entered yet' : '尚未录入任何实盘数量'); return }
    const diffs = (detail.lines ?? []).filter((l) => {
      const c = counts[l.id]?.trim()
      return c !== '' && c !== undefined && Number(c) !== num(l.systemQty)
    }).length
    const confirmMsg = isEn
      ? `${filled} lines entered, ${diffs} with differences. Completing will generate stock adjustments and can't be changed afterwards. Confirm?`
      : `已录入 ${filled} 行，其中 ${diffs} 行有差异。完成后将生成库存调整，且不可再修改。确认完成？`
    ...
    try {
      ...
      setDetail(updated)
      toast.success(isEn ? 'Stock take completed, stock adjusted' : '盘点完成，库存已调整')
      loadList()
    } catch (e) {
      toast.error((e as Error).message)
    } finally {
  ...
  async function cancel() {
    if (!detail || saving) return
    if (!window.confirm(isEn ? `Cancel stock take ${detail.name}?` : `确认取消盘点单 ${detail.name}？`)) return
    setSaving(true)
    try {
      await apiPatch(`/api/stock-takes/${detail.id}`, { action: 'cancel' })
      toast.success(isEn ? 'Cancelled' : '已取消')
      setDetail(null)
      loadList()
    } catch (e) {
      toast.error((e as Error).message)
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <Button variant="outline" size="sm" onClick={() => { setDetail(null); loadList() }}>{isEn ? '← Back to list' : '← 返回列表'}</Button>
            ...
            <span className="text-sm text-gray-500">
              {isEn
                ? `${formatDateOnly(detail.takenAt)} · ${lines.length} products · ${filledCount} entered`
                : `${formatDateOnly(detail.takenAt)} · ${lines.length} 个商品 · 已录 ${filledCount}`}
            </span>
          </div>
          {isDraft && (
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={cancel} disabled={saving}>{isEn ? 'Cancel Stock Take' : '取消盘点'}</Button>
              <Button variant="outline" size="sm" onClick={() => saveCounts()} disabled={saving}>{isEn ? 'Save Draft' : '暂存'}</Button>
              <Button size="sm" style={{ backgroundColor: PURPLE }} className="text-white" onClick={complete} disabled={saving}>
                {isEn ? 'Complete Stock Take' : '完成盘点'}
              </Button>
            </div>
          )}
        </div>
        {detail.notes && <p className="text-sm text-gray-500 mb-3">{isEn ? 'Note' : '备注'}：{detail.notes}</p>}
        <div className="border rounded overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-left">
              <tr>
                <th className="px-3 py-2 font-medium">{isEn ? 'Product' : '商品'}</th>
                <th className="px-3 py-2 font-medium text-right w-32">{isEn ? 'System Stock' : '系统库存'}</th>
                <th className="px-3 py-2 font-medium text-right w-36">{isEn ? 'Counted Qty' : '实盘数量'}</th>
                <th className="px-3 py-2 font-medium text-right w-28">{isEn ? 'Difference' : '差异'}</th>
              </tr>
            </thead>
            <tbody>
              {lines.map((l) => {
                    ...
                    <td className="px-3 py-1.5 text-right">
                      {isDraft ? (
                        <input
                          ...
                          value={counts[l.id] ?? ''}
                          placeholder={isEn ? 'Not counted' : '未盘'}
                          onChange={(e) => setCounts((prev) => ({ ...prev, [l.id]: e.target.value }))}
                        />
                      ) : (
                        <span className="tabular-nums">{counted === null ? (isEn ? 'Not counted' : '未盘') : counted}</span>
                      )}
                    </td>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold">{isEn ? 'Stock Take' : '库存盘点'}</h1>
        <Button style={{ backgroundColor: PURPLE }} className="text-white" onClick={() => setCreateOpen(true)}>
          {isEn ? 'New Stock Take' : '新建盘点单'}
        </Button>
      </div>
      {list.length === 0 ? (
        <div className="text-center text-gray-400 py-16 border rounded">
          {isEn ? 'No stock takes yet. Click "New Stock Take" and pick categories to start counting.' : '还没有盘点单。点右上角「新建盘点单」，按分类圈定范围开始盘点。'}
        </div>
      ) : (
        <div className="border rounded overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-left">
              <tr>
                <th className="px-3 py-2 font-medium">{isEn ? 'Reference' : '编号'}</th>
                <th className="px-3 py-2 font-medium">{isEn ? 'Status' : '状态'}</th>
                <th className="px-3 py-2 font-medium">{isEn ? 'Date' : '盘点日期'}</th>
                <th className="px-3 py-2 font-medium text-right">{isEn ? 'Products' : '商品数'}</th>
                <th className="px-3 py-2 font-medium">{isEn ? 'Created By' : '创建人'}</th>
                <th className="px-3 py-2 font-medium">{isEn ? 'Note' : '备注'}</th>
              </tr>
            </thead>
            <tbody>
              {list.map((st) => (
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{isEn ? 'New Stock Take' : '新建盘点单'}</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
              <Label className="mb-2 block">{isEn ? 'Scope (pick categories, 500 products max per stock take)' : '盘点范围（选分类，单张上限 500 个商品）'}</Label>
              <div className="max-h-56 overflow-y-auto border rounded p-2 space-y-1">
                {categories.map((c) => (
                  <label key={c.id} className="flex items-center gap-2 text-sm cursor-pointer hover:bg-gray-50 px-1 py-0.5 rounded">
                    ...
                    />
                    {isEn ? c.name : (c.nameZh ? `${c.nameZh} (${c.name})` : c.name)}
                  </label>
                ))}
              </div>
            </div>
            <div>
              <Label htmlFor="stk-notes" className="mb-1 block">{isEn ? 'Note (optional)' : '备注（可选）'}</Label>
              <Input id="stk-notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={isEn ? 'e.g. month-end routine stock take' : '如：月末例行盘点'} />
            </div>
          </div>
          <DialogFooter>
            ...
            <Button style={{ backgroundColor: PURPLE }} className="text-white" onClick={handleCreate} disabled={creating}>
              {creating ? (isEn ? 'Creating…' : '创建中…') : (isEn ? 'Create & Start' : '创建并开始盘点')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
