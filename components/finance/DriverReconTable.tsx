...
import { RECON_STATUS_LABEL, RECON_STATUS_LABEL_EN } from '@/lib/driver-reconciliation'
...
function Cmp({ declared, system, diff, money, isEn }: {
  ...
  return (
    <td className={`px-3 py-2 whitespace-nowrap tabular-nums ${bad ? 'bg-red-50 text-red-700 font-medium' : ''}`}>
      ...
      <div className="text-xs text-gray-500">{isEn ? 'System' : '系统'} {fmt(system)}</div>
      {bad && <div className="text-xs font-semibold">{isEn ? 'Diff ' : '差 '}{diff > 0 ? '+' : ''}{fmt(diff)}</div>}
    </td>
  )
}
  ...
  if (rows.length === 0) {
    return (
      <div className="border rounded p-10 text-center text-gray-500">
        {isEn ? 'No matching reconciliation records in this range' : '该区间内没有符合条件的对账记录'}
      </div>
    )
  }
      ...
      <table className="min-w-full text-sm">
        <thead className="bg-gray-50 text-gray-600">
          <tr className="text-left">
            <th className="px-3 py-2 font-medium">{isEn ? 'Business Date' : '业务日'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Driver' : '司机'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Status' : '状态'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Cash Collected' : '收回现金'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Order Total' : '订单总额'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Returns' : '退货'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Exchanges' : '换货'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Trips' : '行程'}</th>
            <th className="px-3 py-2 font-medium">{isEn ? 'Action' : '操作'}</th>
          </tr>
        </thead>
                ...
                <td className="px-3 py-2 whitespace-nowrap">
                  ...
                  {r.hasDiff && (
                    <span className="ml-1 px-2 py-0.5 rounded text-xs bg-red-100 text-red-700">
                      {isEn ? `${r.diffs.length} mismatch${r.diffs.length > 1 ? 'es' : ''}` : `${r.diffs.length} 项对不上`}
                    </span>
                  )}
                </td>
                <Cmp declared={r.declared?.cashCollected ?? null} system={r.system.cashCollected}
                     diff={d('cashCollected')} money isEn={isEn} />
                <Cmp declared={r.declared?.orderTotal ?? null} system={r.system.orderTotal}
                     diff={d('orderTotal')} money isEn={isEn} />
                <Cmp declared={r.declared?.returnCount ?? null} system={r.system.returnCount}
                     diff={d('returnCount')} isEn={isEn} />
                <Cmp declared={r.declared?.exchangeCount ?? null} system={r.system.exchangeCount}
                     diff={d('exchangeCount')} isEn={isEn} />
                <td className="px-3 py-2 whitespace-nowrap text-gray-600">
                  {isEn ? `${r.system.tripIds.length} trips` : `${r.system.tripIds.length} 趟`}
                  {r.system.unsettledTripCount > 0 && (
                    <span className="block text-xs text-amber-700">
                      {isEn ? `${r.system.unsettledTripCount} unsettled` : `${r.system.unsettledTripCount} 趟未交账`}
                    </span>
                  )}
                </td>
                <td className="px-3 py-2 whitespace-nowrap">
                  {r.status === 'submitted' && (
                    ...
                    >
                      {acting === key ? (isEn ? 'Confirming…' : '确认中…') : (isEn ? 'Confirm' : '确认')}
                    </button>
                  )}
                  {r.status === 'confirmed' && (
                    <span className="text-xs text-gray-500">
                      {r.confirmedByName || (isEn ? 'Confirmed' : '已确认')}
                    </span>
                  )}
                  {r.status === 'not_submitted' && (
                    <span className="text-xs text-gray-400">{isEn ? 'Awaiting driver submission' : '待司机提交'}</span>
                  )}
                </td>
              </tr>
            )
